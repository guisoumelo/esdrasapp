...
export function TimelineBlock({ label, timeLabel, state, isLast = false, onPress }: TimelineBlockProps) {
  const colors = useColors();
  const sc = getStateColors(state, colors);
  const tappable = state !== 'locked' && onPress != null;
    ...
    <View style={styles.wrapper}>
      <View style={styles.row}>
        ...
        <View style={styles.track}>
          <View style={[styles.dot, { backgroundColor: sc.bg, borderColor: sc.border }]}>
        ...
        {/* Content card — tappable when not locked */}
        <TouchableOpacity
          style={[
            styles.card,
            { backgroundColor: colors.card, borderColor: sc.border },
            tappable && styles.cardTappable,
          ]}
        ...
        >
          ...
          {tappable && (
            <Text style={[styles.tapHint, { color: sc.border }]}>
              Toque para ir →
            </Text>
          )}
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  dotIcon: { fontSize: 11, fontWeight: 'bold' },
  ...
  tapHint: { fontSize: 11, marginTop: 5, fontWeight: '600' },
});
